...
class DefaultExtension extends MProvider {
  ...
  async getPopular(page) {
    return await this.getManga(
      `/api/v3/comics?free_type=1&limit=16&offset=${
        (page - 1) * 16
      }&ordering=-popular&_update=true`
    );
  }
  ...
  async getLatestUpdates(page) {
    return await this.getManga(
      `/api/v3/comics?free_type=1&limit=16&offset=${
        (page - 1) * 16
      }&ordering=-datetime_updated&_update=true`
    );
  }
  ...
  async search(query, page, filters) {
    if (query != "") {
      const res = await new Client().get(
        this.source.apiUrl +
          `/api/v3/search/comic?platform=1&q=${query}&limit=16&offset=${
            (page - 1) * 16
          }&q_type=&_update=true`,
        {
          webp: "1",
    ...
    return await this.getManga(
      `/api/v3/comics?free_type=1&limit=16&offset=${
        (page - 1) * 16
      }&theme=${type}&top=${region}&ordering=${sort}&_update=true`
    );
  }
