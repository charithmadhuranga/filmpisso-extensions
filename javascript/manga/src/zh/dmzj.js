...
class DefaultExtension extends MProvider {
  ...
  async search(query, page, filters) {
    if (query == "") {
      ...
      return await this.getManga(
        `https://m.idmzj.com/classify/${type}-0-${status}-${region}-${sort}-${
          page - 1
        }.json`
      );
    }
  ...
  async getDetail(url) {
    ...
    const res = await new Client().get(
      `https://www.dmzj.com/api/v1/comic1/comic/detail?comic_py=${url}&channel=pc&app_name=dmzj&version=1.0.0&uid=${preference.get(
        "uid"
      )}`
    );
    const datas = JSON.parse(res.body);
  ...
  async getPageList(url) {
    ...
    const ids = url.split("|");
    const res = await new Client().get(
      `https://www.dmzj.com/api/v1/comic1/chapter/detail?channel=pc&app_name=dmzj&version=1.0.0&comic_id=${
        ids[0]
      }&chapter_id=${ids[1]}&uid=${preference.get("uid")}`
    );
    const datas = JSON.parse(res.body);
    if (datas["errno"] != 0) {
      return [];
