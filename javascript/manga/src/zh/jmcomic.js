...
class DefaultExtension extends MProvider {
  ...
  async getPopular(page) {
    return await this.getManga(
      `/albums?t=a&o=mv&page=${page}`,
      "div.thumb-overlay-albums"
    );
  }

  async getLatestUpdates(page) {
    return await this.getManga(
      `/albums?t=a&o=mr&page=${page}`,
      "div.thumb-overlay-albums"
    );
  }
  ...
  async search(query, page, filters) {
    ...
    if (query != "") {
      return await this.getManga(
        `/search/photos?search_query=${query}&t=a&o=mr`,
        "div.thumb-overlay"
      );
    }
    return await this.getManga(
      `/albums${"/" + type}?t=${time}&o=${sort}&page=${page}`,
      "div.thumb-overlay-albums"
    );
  }
  ...
  async getDetail(url) {
    ...
    const doc = new Document(res.body);
    const cover = doc
      .selectFirst("div.show_zoom")
      .innerHtml.match(/data-cfsrc="(.*)" data-cfstyle/)[1];
