const filmpissoSources = [
  {
    ...
    apiUrl: "",
    iconUrl:
      "https://raw.githubusercontent.com/charithmadhuranga/filmpisso-extensions/main/javascript/icon/zh.manhuadb.png",
    typeSource: "single",
    isManga: true,
    isNsfw: false,
    version: "0.0.25",
...
class DefaultExtension extends MProvider {
  ...
  async getMangas(url, search) {
    ...
    for (const item of items) {
      const cover = this.coverUrlConvert(
        item.selectFirst("a.d-block img").attr("src")
      );
      var title;
      if (search) {
  ...
  async getDetail(url) {
    ...
    const cover = this.coverUrlConvert(
      doc.selectFirst("td.comic-cover img").attr("src")
    );
