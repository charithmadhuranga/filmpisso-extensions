...
class DefaultExtension extends MProvider {
  ...
  async getFavorites(page) {
    const res = await this.request(
      `users-users_fav-page-${page}-c-0.html`,
      true
    );
  ...
  async getMangas(url_str, page) {
    ...
    for (const element of elements) {
      ...
      const cover =
        "https:" + element.selectFirst("div.pic_box a img").attr("src");
      mangas.push({
        name: title,
        link: url,
        imageUrl: cover,
  ...
  async getDetail(url) {
    ...
    const desc =
      desc_.join("\n").replaceAll("+TAG\n", "").slice(0, -1) +
      doc.selectFirst("div.uwconn p").text;
    ...
    const uploaddate = this.dateStringToTimestamp(
      doc.selectFirst("div.info_col").text.replace("上傳於", "")
    );
    return {
      name: title,
      imageUrl: cover,
  ...
  async getPageList(url) {
    ...
    const html = res.body;
    const urls = [];
    let urls_str = html.substring(
      html.search("imglist") + 12,
      html.search("喜歡紳士漫畫的同學請加入收藏哦！") + 17
    );
