const mangayomiSources = [
  {
    ...
    apiUrl: "",
    iconUrl:
      "https://raw.githubusercontent.com/charithmadhuranga/filmpisso-extensions/main/javascript/icon/de.aniworld.png",
    typeSource: "single",
    isManga: false,
    isNsfw: false,
    version: "0.0.1",
...
class DefaultExtension extends MProvider {
  ...
  async search(query, page, filters) {
    ...
    for (const element of elements) {
      ...
      const img = new Document((await new Client().get(baseUrl + link)).body)
  ...
  episodeFromElement(element) {
    ...
    if (
      element
        .selectFirst("td.seasonEpisodeTitle a")
        .attr("href")
        .includes("/film")
    ) {
      ...
      name =
        `Film ${num}` +
        " : " +
        element.selectFirst("td.seasonEpisodeTitle a span").text;
      url = element.selectFirst("td.seasonEpisodeTitle a").attr("href");
    } else {
      const season = element
        .selectFirst("td.seasonEpisodeTitle a")
        .attr("href")
        .substringAfter("staffel-")
        .substringBefore("/episode");
      const num = element.attr("data-episode-season-id");
      name =
        `Staffel ${season} Folge ${num}` +
        " : " +
        element.selectFirst("td.seasonEpisodeTitle a span").text;
      url = element.selectFirst("td.seasonEpisodeTitle a").attr("href");
    }
  ...
  async getVideoList(url) {
    ...
    for (const element of redirectlink) {
      ...
      if (hoster == "Streamtape" && hosterSelection.includes("Streamtape")) {
        ...
        const vids = await streamTapeExtractor(
          body.match(/https:\/\/streamtape\.com\/e\/[a-zA-Z0-9]+/g)[0],
          quality
        );
        ...
        const vids = await voeExtractor(
          body.match(/https:\/\/voe\.sx\/e\/[a-zA-Z0-9]+/g)[0],
          quality
        );
        for (const vid of vids) {
          videos.push(vid);
  ...
  sortVideos(videos) {
    ...
    videos.sort((a, b) => {
      let qualityMatchA = 0;
      if (a.quality.includes(hoster) && a.quality.includes(subPreference)) {
        qualityMatchA = 1;
      }
      let qualityMatchB = 0;
      if (b.quality.includes(hoster) && b.quality.includes(subPreference)) {
        qualityMatchB = 1;
      }
    ...
    return [
      {
        ...
        listPreference: {
          ...
          summary: "",
          valueIndex: 0,
          entries: ["Deutscher Sub", "Deutscher Dub", "Englischer Sub"],
          entryValues: ["Deutscher Sub", "Deutscher Dub", "Englischer Sub"],
        },
      },
      {
        ...
        listPreference: {
          ...
          summary: "",
          valueIndex: 0,
          entries: ["Streamtape", "VOE", "Vidoza"],
          entryValues: ["Streamtape", "VOE", "Vidoza"],
        },
      },
      {
