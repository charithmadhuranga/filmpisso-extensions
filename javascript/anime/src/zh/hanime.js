...
class DefaultExtension extends MProvider {
  ...
  async getItems(url, type) {
    ...
    if (type == 0) {
      ...
      const elements = doc
        .selectFirst("div.content-padding-new")
        .selectFirst("div.row")
        .select("div.col-xs-6");
      for (const element of elements) {
  ...
  async getPopular(page) {
    return await this.getItems(
      `/search?sort=%E8%A7%80%E7%9C%8B%E6%AC%A1%E6%95%B8&page=${page}`,
      1
    );
  }
  ...
  async getLatestUpdates(page) {
    return await this.getItems(
      `/search?sort=%E6%9C%80%E6%96%B0%E4%B8%8A%E5%B8%82&page=${page}`,
      1
    );
  }
  ...
  async search(query, page, filters) {
    ...
    return await this.getItems(
      `/search?query=${query}${url_str}&page=${page}`,
      type
    );
  }
  ...
  async getDetail(url) {
    ...
    const desc = doc.selectFirst(
      "div.video-details-wrapper div.video-caption-text"
    ).text;
    const adddate = this.dateStringToTimestamp(
      doc.selectFirst("div.video-details-wrapper").text.split("  ")[1]
    );
    const author = doc
      .selectFirst("a#video-artist-name")
      .text.replaceAll("\n", "")
      .trim(" ");
    const tags = doc
      .select("div.single-video-tag a ")
      .map((e) => e.text)
      .slice(0, -2);
    const eps_ = doc
      .selectFirst("div#video-playlist-wrapper")
      .select("div#playlist-scroll div.related-watch-wrap");
    const eps = [];
