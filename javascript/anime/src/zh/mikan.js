...
class DefaultExtension extends MProvider {
  ...
  async getItems(url, cookies) {
    ...
    if (cookies) {
      res = await new Client().get(this.source.baseUrl + url, {
        Cookie: `.AspNetCore.Identity.Application=${new SharedPreferences().get(
          "cookies"
        )}`,
      });
    } else {
    ...
    for (const element of elements) {
      ...
      const cover =
        this.source.baseUrl + element.selectFirst("img").attr("data-src");
      items.push({
        name: title,
        imageUrl: cover,
        link: url,
  ...
  async search(query, page, filters) {
    const res = await new Client().get(
      this.source.baseUrl + `/Home/Search?searchstr=${query}`
    );
    const doc = new Document(res.body);
    const items = [];
    ...
    for (const element of elements) {
      ...
      const cover =
        this.source.baseUrl + element.selectFirst("span").attr("data-src");
      const url = element.selectFirst("a").attr("href");
      items.push({
        name: title,
        imageUrl: cover,
  ...
  async getDetail(url) {
    ...
    const doc = new Document(res.body);
    const cover =
      this.source.baseUrl + doc.selectFirst("div.content img").attr("src");
    ...
    for (const list of lists) {
      ...
      for (const item of list.select("div.m-bangumi-item")) {
        ...
        const url =
          this.source.baseUrl + item.selectFirst("div.right a").attr("href");
