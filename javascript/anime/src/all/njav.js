...
class DefaultExtension extends MProvider {
  ...
  async request(url) {
    ...
    const res = await new Client().get(
      preference.get("url") + "/" + preference.get("lang") + url
    );
    return res.body;
  }
  ...
  async getDetail(url) {
    ...
    const updateTime = this.dateStringToTimestamp(
      info[1].select("span")[1].text
    );
    var author;
    try {
